export const PRESET_PROBLEMS = [
  ["4k4/9/9/9/9/9/9/9/4K4 b 2r2b4g4s4n4l18p", "双玉のみ"],
  ["4k4/9/9/9/9/9/9/9/9 b 2r2b4g4s4n4l18p 1", "単玉のみ"],
  [
    "5bllb/SGG1g1+R+RP/1Lns1sK1s/1pp1p+p2g/3p1+p3/5k2+p/1P4n2/P3+pp+plN/2P1P+p2+p b NP 1",
    "森茂「龍の顎」 12555手",
  ],
  [
    "l5bgg/1KB1r2sL/1GL3sr1/k1LG1sp1P/1nnNsp1P1/Pn2p1P2/2PP1P3/1P2P4/9 b 6p 1",
    "加藤徹「寿限無」 19447手",
  ],
  [
    "lr1B2Kpl/1L1S5/1GNgR1Ls1/k1Nb2s1P/1nppns1P1/Pg2g1P2/2PP1P3/1P2P4/9 b 6p 1",
    "加藤徹「寿限無2」 38889手",
  ],
  [
    "rBKp1r1ss/GL3p1bL/L3klpp1/2G2n2P/1gnnng1P1/P1s1s1P2/1P1P1P3/2P1P4/9 b 5p 1",
    "加藤徹「寿限無3」 49909手",
  ],
  [
    "1K1R1r1S1/L1+P1l1L+NB/4lg1N1/1nbs2g1P/1gpn1g1P1/1Pk1s1P2/3P1P3/4+p4/P1s1P4 b 2P5p",
    "森茂・橋本孝治・浦壁和彦「寿限無」完全限定化案 36511手",
  ],
  [
    "1K1R1r1S1/L2+Pl1L+NB/3+P+ng1N1/1nps2g1P/1gbl1g1P1/1kP1s1P2/3P1P3/4+p4/P1s1P4 b P5p 1",
    "ogiekako「寿限無」完全限定化案の改案 36607手",
  ],
  // TODO: make them solvable
  //   [
  //     "r3+B1K2/SG4b2/1L2N3l/1GNGLp3/2rNp1n1g/1ss4lk/5P3/3PP1P1P/PPP3sP1 b 7p 1",
  //     "ogiekako「BATACO」 4247手",
  //   ],
  //   [
  //     "8l/l1SS1PGP1/1PPNP1Pp1/4b2sk/Rg7/2s1gn2P/3nN4/1lg3l2/K1+B3r2 b 10P 1",
  //     "ogiekako「チェイン」 207手",
  //   ],
] as const;
